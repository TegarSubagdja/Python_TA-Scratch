...
# Fungsi untuk menghitung jarak Euclidian
def euclidean(pos, target):
    return np.linalg.norm(np.array(pos) - np.array(target))

# Fungsi untuk menormalisasi angle -phi -> phi
def normalize_angle(angle):
    return np.arctan2(np.sin(angle), np.cos(angle))

# Fungsi untuk mencari center
def midpoint(p1, p2):
    return ((p1[0]+p2[0])//2, (p1[1]+p2[1])//2)

# Fungsi untuk mencari center sisi atas dan bawah
