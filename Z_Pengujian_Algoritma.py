...
def runPengujianAvgLengthOneScenario(size = [16, 32, 64, 128]):
    ...
    for i in range(5):
        ...
        for sz in size:
            ...
            for _ in range(10):
                (path, times), openlist, closelist = Algoritm(
                    ...
                    BRC=BRC, TPF=TPF, PPO=PPO, ELL=ELL,
            ...
            print(f"{sz} {method_name} {mapChoice} - mean_time: {np.mean(tempTimes):.4f}")
        ...
        with pd.ExcelWriter(f"Excel/ELL/Hasil_Pengujian_{nameMap}_{size[-1]}_avg_length.xlsx") as writer:
...
if __name__ == "__main__":
    ...
    # # runPengujianAvgLengthOneScenario()
    # sys.exit()
